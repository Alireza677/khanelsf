...
@php
    ...
    $hasHoverSubmenu = (! $subGrouped) && blank($url) && filled($childItems);
@endphp
...
<li
    @if ($hasHoverSubmenu)
        x-data="{
            ...
            sidebarWasOpen: true,
            openSubmenu() {
                this.sidebarWasOpen = $store.sidebar.isOpen;

                if (window.matchMedia('(min-width: 1024px)').matches && ! this.sidebarWasOpen) {
                    $store.sidebar.open();
                }
        ...
        x-on:focusin="if (! window.matchMedia('(pointer: coarse)').matches) openSubmenu()"
        x-on:focusout="if (! $el.contains($event.relatedTarget)) closeSubmenu()"
    @endif
    {{
        $attributes->class([
            'fi-sidebar-item',
            'cms-sidebar-parent-item' => $hasHoverSubmenu,
            // @deprecated `fi-sidebar-item-active` has been replaced by `fi-active`.
            'fi-active fi-sidebar-item-active' => $active,
            'flex flex-col gap-y-1' => $active || $activeChildItems || $hasHoverSubmenu,
        ])
    }}
>
    <a
        ...
        @if ($hasHoverSubmenu)
            ...
            x-on:click.prevent.stop="submenuOpen ? closeSubmenu() : openSubmenu()"
            x-on:keydown.enter.prevent="submenuOpen ? closeSubmenu() : openSubmenu()"
            x-on:keydown.space.prevent="submenuOpen ? closeSubmenu() : openSubmenu()"
        @else
        ...
        @class([
            ...
            'cms-sidebar-parent-trigger cursor-pointer hover:bg-gray-100 focus-visible:bg-gray-100 dark:hover:bg-white/5 dark:focus-visible:bg-white/5' => $hasHoverSubmenu,
    ...
    >
        ...
        @if ($hasHoverSubmenu)
            <x-filament::icon
                ...
                x-show="$store.sidebar.isOpen"
                x-bind:class="{ 'rotate-180': submenuOpen }"
                class="cms-sidebar-parent-chevron h-4 w-4 text-gray-400 transition-transform dark:text-gray-500"
            />
        @endif
    ...
    @if (($active || $activeChildItems || $hasHoverSubmenu) && $childItems)
        <ul
            @if ($hasHoverSubmenu)
                x-cloak
                x-show="submenuOpen"
                x-collapse.duration.150ms
                class="fi-sidebar-sub-group-items cms-sidebar-submenu flex flex-col gap-y-1"
            @else
        ...
        >
            @foreach ($childItems as $childItem)
                <x-filament-panels::sidebar.item
                    ...
                    :sidebar-collapsible="$hasHoverSubmenu ? false : $sidebarCollapsible"
                    sub-grouped
